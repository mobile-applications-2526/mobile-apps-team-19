...
export function CreateEventForm({ onSuccess, onCancel }: CreateEventFormProps) {
    ...
    const handleSubmit = async () => {
        ...
        if (!timeRegex.test(startTime)) {
            Alert.alert('Error', 'Invalid start time format. Use HH:MM (e.g., 18:00)');
            return;
        }
        ...
        const eventData: CreateEventData = {
            ...
            startTime: startTime.includes(':') && startTime.split(':').length === 2
                ? `${startTime}:00`
                : startTime,
            endTime: endTime.includes(':') && endTime.split(':').length === 2
                ? `${endTime}:00`
                : endTime,
    ...
    return (
        <ScrollView style={[styles.container, { backgroundColor: colors.background }]}>
            <View style={styles.form}>
                ...
                <View style={styles.inputGroup}>
                    <Text style={[styles.label, { color: colors.text }]}>Host Name *</Text>
                    <TextInput
                        style={[styles.input, {
                            backgroundColor: colors.background,
                            color: colors.text,
